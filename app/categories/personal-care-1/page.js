...
function PersonalCareContent() {
  ...
  const subCategories = [
    { name: "Face Care", count: 450 },
    { name: "Body Care", count: 320 },
    { name: "Hair Care", count: 280 },
    { name: "Oral Care", count: 150 },
    { name: "Baby Care", count: 210 },
    { name: "Male Grooming", count: 180 },
    { name: "Sanitary Pads", count: 120 },
    { name: "Hand Wash", count: 90 },
  ];

  const filters = [
    { name: "Brand", options: ["Himalaya", "Cetaphil", "Mamaearth", "Nivea", "Dove", "Ponds", "L'Oreal", "Garnier"] },
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/products?category=Personal%20Care", { cache: 'no-store' });
        ...
        if (data.success && data.data.length > 0) {
          ...
          const mockProducts = [
            { _id: 1, name: "Cetaphil Gentle Skin Cleanser 250ml", brand: "Cetaphil", price: 535, mrp: 595, discount: "10%", rating: 4.8, reviews: 1250, image: "https://assets.truemeds.in/Images/HomepageImage/Picture_1698220522992_125.png", stock: 100 },
            { _id: 2, name: "Himalaya Purifying Neem Face Wash 200ml", brand: "Himalaya", price: 180, mrp: 220, discount: "18%", rating: 4.5, reviews: 850, image: "https://assets.truemeds.in/Images/HomepageImage/Picture_1698220590032_127.png", stock: 100 },
            { _id: 3, name: "Mamaearth Onion Hair Fall Shampoo 250ml", brand: "Mamaearth", price: 315, mrp: 349, discount: "10%", rating: 4.6, reviews: 620, image: "https://assets.truemeds.in/Images/HomepageImage/Picture_1741163760626_122.png", stock: 100 },
            { _id: 4, name: "Nivea Soft Light Moisturiser 200ml", brand: "Nivea", price: 285, mrp: 325, discount: "12%", rating: 4.7, reviews: 2100, image: "https://assets.truemeds.in/Images/HomepageImage/Picture_1756443365418_123.png", stock: 100 },
            { _id: 5, name: "Dove Cream Beauty Bathing Bar 125g (Pack of 3)", brand: "Dove", price: 210, mrp: 240, discount: "12%", rating: 4.9, reviews: 3400, image: "https://assets.truemeds.in/Images/HomepageImage/Picture_1698220598279_128.png", stock: 100 },
            { _id: 6, name: "Colgate MaxFresh Toothpaste 150g", brand: "Colgate", price: 95, mrp: 110, discount: "13%", rating: 4.4, reviews: 1500, image: "https://assets.truemeds.in/Images/HomepageImage/Picture_1698220496491_124.png", stock: 100 },
          ];
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  {subCategories.map((sub) => (
                    <label key={sub.name} className="flex items-center justify-between group cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className={`text-sm transition-colors ${selectedSubCats.includes(sub.name) ? "text-blue-600 font-bold" : "text-gray-600 group-hover:text-blue-600"}`}>{sub.name}</span>
                      </div>
                    ...
                    {filter.options.map((opt) => (
                      <label key={opt} className="flex items-center gap-3 group cursor-pointer">
                        <input 
                          ...
                          className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" 
                        />
                        <span className={`text-sm transition-colors ${(filter.name === "Brand" && selectedBrands.includes(opt)) ? "text-blue-600 font-bold" : "text-gray-600 group-hover:text-blue-600"}`}>{opt}</span>
                      </label>
                    ))}
                  </div>
                </div>
          ...
          <div className="flex-1">
            ...
            <div className="lg:hidden flex overflow-x-auto gap-3 pb-6 no-scrollbar -mx-4 px-4">
              {subCategories.map((sub) => (
                <button 
                  ...
                  className={`shrink-0 px-4 py-2 border rounded-full text-xs font-bold whitespace-nowrap transition-all ${selectedSubCats.includes(sub.name) ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-gray-200 text-gray-700 active:bg-blue-50"}`}
                >
                  {sub.name}
                </button>
              ))}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              <div>
                <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-1">Personal Care</h1>
                <p className="text-sm text-gray-500">Showing 1 - {filteredProducts.length} of {products.length} products</p>
              </div>
            ...
            <div className={`fixed inset-0 z-[100] lg:hidden transition-opacity duration-300 ${showMobileFilters ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
              ...
              <div className={`absolute inset-y-0 right-0 w-[320px] bg-white shadow-2xl transform transition-transform duration-300 flex flex-col ${showMobileFilters ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-3">
                  ...
                  <button onClick={() => setShowMobileFilters(false)} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-100">Apply</button>
                </div>
              </div>
            </div>
              ...
              {loading ? (
                ...
                filteredProducts.map((product) => (
                  ...
                  >
                    ...
                    <h3 className="text-xs md:text-sm font-bold text-gray-800 line-clamp-2 mb-1 group-hover:text-blue-600 transition-colors h-8 md:h-10 leading-tight">{product.name}</h3>
    ...
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-bold">Loading Personal Care...</p>
        </div>
      </div>
    }>
      <PersonalCareContent />
